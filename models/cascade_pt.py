...
class Model():
    ...
    def build_net(self):
        ...
        self.render_layer = render.RenderLayerPointLightTorch()
        ...
        self.optimizerD_brdf = torch.optim.Adam(self.decoderRef_brdf.parameters(), lr=2e-4, betas=(0.5, 0.999))
        self.optimizerD_render = torch.optim.Adam(self.decoderRef_render.parameters(), lr=2e-4, betas=(0.5, 0.999))
    ...
    def compute_loss(self):
        ...
        self.loss_relit = torch.sum((self.image_caspred - self.image_t) ** 2 * self.mask.expand_as(self.image_t)) / pixel_num / 3.0
    ...
    def save_cur_sample(self, epoch):
        path = '%s/%s/cas1/state_dict_%s/samples' % (self.opts.outf, self.name, str(epoch))
        if not os.path.exists(path):
            os.makedirs(path)
        ...
        vutils.save_image((((self.image_render+1.0)/2.0)*self.mask.expand_as((self.image_s))**(1.0/2.2)).data,
                    '{0}/image_render.png'.format(path))
    ...
    def save_error_to_file(self, epoch):
        path = '%s/%s/cas1/state_dict_%s/errors' % (self.opts.outf, self.name, str(epoch))
        if not os.path.exists(path):
            os.makedirs(path)
    ...
    def save_cur_checkpoint(self, epoch):
        ...
        path = '%s/%s/cas1/state_dict_%s/models' % (self.opts.outf, self.name, str(epoch))
        if not os.path.exists(path):
            os.makedirs(path)
