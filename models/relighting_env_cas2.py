...
class Model():
    ...
    def build_net(self):
        ...
        self.decoderRef2_render = nn.DataParallel(network.RefineDecoderRender(litc=30), device_ids=self.opts.gpu_id).cuda()
        self.env_cas2predictor = nn.DataParallel(network.RefineDecoderEnv(), device_ids=self.opts.gpu_id).cuda()
        ...
        self.optimizerBRDF = torch.optim.Adam(self.decoderRef2_brdf.parameters(), lr=2e-4, betas=(0.5, 0.999))
        self.optimizerDRen = torch.optim.Adam(self.decoderRef2_render.parameters(), lr=2e-4, betas=(0.5, 0.999))
        self.optimizerEnv = torch.optim.Adam(self.env_cas2predictor.parameters(), lr=2e-4, betas=(0.5, 0.999))
    ...
    def forward(self):
        ...
        # render
        image_pt = self.render_layer.forward_batch(self.albedo_pred_cas, self.normal_pred_cas, self.rough_pred_cas, self.depth_pred_cas, self.mask, self.light_t)
        image_env = self.render_layer.forward_env(self.albedo_pred_cas, self.normal_pred_cas, self.rough_pred_cas, self.mask, self.env_pred_cas)
        relit_render = 2 * (image_pt + image_env) - 1
        self.relit_render_cas = torch.clamp_(relit_render, -1, 1) * self.mask

        # ----------------- Cas2 -----------------
        cas2_input = torch.cat([self.image_s_pe, \
                               self.relit_caspred * self.mask, \
                               self.relit_render_cas * self.mask, \
        ...
        feat_cas2 = self.encoderRef2(cas2_input)
        ...
        self.env_pred_cas2 = self.env_cas2predictor(feat_cas2[-1], self.env_pred_cas)
        light_t = torch.cat([self.light_t, self.env_pred_cas2.view(self.env_pred_cas2.size(0), -1)], 1) 
        self.relit_cas2pred = self.decoderRef2_render(feat_cas2, brdf_feat_cas2, light_t) * self.mask

    def compute_loss(self):
        pixel_num = (torch.sum(self.mask).cpu().data).item()

        self.loss_relit = torch.sum((self.relit_cas2pred - self.image_t) ** 2 * self.mask.expand_as(self.image_t)) / pixel_num / 3.0
        ...
        self.lossE = torch.mean((self.SH - self.env_pred_cas2) ** 2)
    ...
    def save_cur_sample(self, epoch):
        ...
        vutils.save_image((((self.relit_caspred+1.0)/2.0 * self.mask + self.image_bg)**(1.0/2.2)).data,
                    '{0}/relit_caspred.png'.format(path))
        vutils.save_image((((self.relit_cas2pred+1.0)/2.0 * self.mask + self.image_bg)**(1.0/2.2)).data,
                    '{0}/relit_caspred2.png'.format(path))
        ...
        vutils.save_image((0.5*(self.albedo_pred_cas2 + 1)*self.mask.expand_as(self.albedo)).data,
                    '{0}/albedo_pred.png'.format(path))
        vutils.save_image((0.5*(self.normal_pred_cas2 + 1)*self.mask.expand_as(self.normal)).data,
                    '{0}/normal_pred.png'.format(path))
        vutils.save_image((0.5*(self.rough_pred_cas2  + 1)*self.mask.expand_as(self.rough )).data,
                    '{0}/rough_pred.png'.format(path))
        depth = 1 / torch.clamp(self.depth_pred_cas2, 1e-6, 10) * self.mask.expand_as(self.depth)
        depth = (depth - 0.25) / 0.8
    ...
    def save_cur_checkpoint(self, epoch):
        ...
        torch.save(self.env_cas2predictor.state_dict(), '%s/env_cas2predictor.pth' % path)
        print('--> saving done')
    ...
    def load_saved_checkpoint(self, start_epoch):
        ...
        self.env_cas2predictor.load_state_dict(torch.load('%s/env_cas2predictor.pth' % path, map_location=lambda storage, loc:storage))
