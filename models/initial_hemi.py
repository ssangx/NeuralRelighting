...
class Model():
    ...
    def set_input_var(self, data):
        ...
        for i in range(self.aux_cnt):
            self.aux_lights.append(self.gen_light_batch(self.albedo.size(0)))
            self.aux_images.append(2 * self.render_layer.forward_batch(self.albedo, \
                                                                       self.normal, \
                                                                       self.rough, \
                                                                       self.depth, \
                                                                       self.mask, \
    ...
    def forward(self):
        ...
        for i in range(len(self.aux_lights)):
            self.aux_preds += [self.decoder_render(feat, brdf_feat, self.aux_lights[i])]
    ...
    def compute_loss(self):
        ...
        loss_relit = []
        for i in range(len(self.aux_lights)):
            loss_relit.append(torch.sum((self.aux_preds[i] - self.aux_images[i]) ** 2 * self.mask.expand_as(self.image_s)) / pixel_num / 3.0)
    ...
    def save_cur_sample(self, epoch):
        ...
        for i, (img_pred, img_target) in enumerate(zip(self.aux_preds, self.aux_images)):
            vutils.save_image((((img_pred+1.0)/2.0)*self.mask.expand_as(self.image_s)**(1.0/2.2)).data,
                 '{0}/image_pred_{1}.png'.format(path, i))
            vutils.save_image((((img_target+1.0)/2.0)*self.mask.expand_as(self.image_s)**(1.0/2.2)).data,
